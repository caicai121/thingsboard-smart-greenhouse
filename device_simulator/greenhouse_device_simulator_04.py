...
ACCESS_TOKEN = os.getenv("TB_DEVICE_04_TOKEN", os.getenv("ACCESS_TOKEN", "YOUR_ACCESS_TOKEN"))
...
def main():
    ...
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [DEVICE_04] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    )

    if ACCESS_TOKEN == "YOUR_ACCESS_TOKEN":
        logging.error("请在 .env 文件中配置 TB_DEVICE_04_TOKEN")
        logging.error("复制 .env.example 为 .env 并填入真实 Token")
        return

    client = mqtt.Client(
        client_id="greenhouse_device_04_simulator",
    ...
    logging.info(f"[MAIN] PID={os.getpid()} Greenhouse Device 04 simulator started")
