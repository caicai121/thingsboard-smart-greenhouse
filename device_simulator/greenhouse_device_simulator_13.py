...
ACCESS_TOKEN = os.getenv("TB_DEVICE_13_TOKEN", os.getenv("ACCESS_TOKEN", "YOUR_ACCESS_TOKEN"))
...
def main():
    ...
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [DEVICE_13] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    )

    if ACCESS_TOKEN == "YOUR_ACCESS_TOKEN":
        logging.error("请在 .env 文件中配置 TB_DEVICE_13_TOKEN")
        logging.error("复制 .env.example 为 .env 并填入真实 Token")
        return

    client = mqtt.Client(
        client_id="greenhouse_device_13_simulator",
    ...
    logging.info(f"[MAIN] PID={os.getpid()} Greenhouse Device 13 simulator started")
